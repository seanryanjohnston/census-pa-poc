...
The source profile and stage QA are regenerated by the replay commands. The
retained final population acceptance evidence is
...
`POC029` is accepted and direct legislative parity is documented. The later
direct-only pivot removed the superseded precinct workflow from the active
surface; its history remains recoverable from Git.
